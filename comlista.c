...
int removeLista(Processos *l, int pid);

int addLista(Processos *l);
...
{	
	...
	*j = 0;
	...
	//FAZER FOR INICIALIZANDO PRIORIDADES COM I PARA LISTA PARA TESTAR
...
int addLista(Processos *l)
{
	...
	{
		novo->pid = l->pid;
		strcpy(novo->path, l->path);
		novo->prioridade = l->prioridade;
		novo->prox = novo;
	}
